...
gulp.task('rename', function ()  {
    var newname;
    var author = 'Fikret Causevic';
    var email  = 'causch@gmx.de';
    ...
    if (!newname) {
        console.log('Please write the new network name, like: "gulp rename --name mywidgetset" --author "Author Name"');
        process.exit();
    }
    ...
    if (fs.existsSync(__dirname + '/admin/network.png')) {
        fs.renameSync(__dirname + '/admin/network.png',              __dirname + '/admin/' + newname + '.png');
    }
    if (fs.existsSync(__dirname + '/widgets/network.html')) {
        fs.renameSync(__dirname + '/widgets/network.html',           __dirname + '/widgets/' + newname + '.html');
    }
    if (fs.existsSync(__dirname + '/widgets/network/js/network.js')) {
        fs.renameSync(__dirname + '/widgets/network/js/network.js', __dirname + '/widgets/network/js/' + newname + '.js');
    }
    if (fs.existsSync(__dirname + '/widgets/network')) {
        fs.renameSync(__dirname + '/widgets/network',                __dirname + '/widgets/' + newname);
    }
    var patterns = [
        {
            match: /network/g,
        ...
        {
            match: /Network/g,
            replacement: newname ? (newname[0].toUpperCase() + newname.substring(1)) : 'Network'
        },
        {
            match: /Fikret Causevic/g,
            replacement: author
        },
        {
            match: /causch@gmx.de/g,
            replacement: email
        }
    ];
    var files = [
